...
public: Stitch_npc_ai_chaman() : CreatureScript("Stitch_npc_ai_chaman") { }
		...
		struct Stitch_npc_ai_chamanAI : public ScriptedAI
		{
			...
			uint32 BrancheSpe = 1;													// Choix de la Specialisation : Restauration=1, Elementaire=2, Amelioration=3
			...
			uint32 Rappel_totemique = 36936;										// Rappel totemique : retire les totems invoques
			...
			uint32 branche2_1[4] = { 9532, 9532, 9532, 28167 };						// Eclair 9532 - Chaîne d'eclairs 28167 
			...
			void Init_AI()
			{
				...
				case 1: // Si Restauration --------------------------------------------------------------------------------------------------------------------------
					...
					// Tirages aleatoires des spells Restauration 
					...
					// Totem
					me->CastSpell(me, Totem_guerisseur, true); 									// Totem guerisseur 5394 (12s)
					
					// Reste a distance variable suivant ci le mob est a l'exterieur ou a l'interrieur
			...
			void UpdateAI(uint32 diff) override
			{
				...
				if (UpdateVictim() /*&& !me->HasUnitState(UNIT_STATE_MOVE) || me->HasUnitState(UNIT_STATE_CASTING)*/)
				{
					...
					switch (BrancheSpe)
					{
					case 1: // Specialisation Restauration ##########################################################################################################
					...
					case 2: // Specialisation Elementaire #############################################################################################################
							// Regen mana en combat -------------------------------------------------------------------------------------------------------------------
						if (Cooldown_RegenMana <= diff)
						{
							me->SetPower(POWER_MANA, Mana + (MaxMana / 5));
					...
					case 3: // Specialisation Amelioration #############################################################################################################
							// Regen mana en combat ------------------------------------------------------------------------------------------------------------------------
						if (Cooldown_RegenMana <= diff)
						{
